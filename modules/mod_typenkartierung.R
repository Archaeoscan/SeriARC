...
mod_typenkartierung_server <- function(meta_data, data_raw, mapping_meta, site_clusters, cluster_names = NULL, cluster_colors = NULL, input, output, session, tr = function(x) x) {
  ...
  # PNG/SVG Export via webshot2 (1:1 Leaflet-Karte)
  output$download_type_map_png <- create_leaflet_png_handler(
    reactive({ build_export_type_map() }), "SeriARC_Typenkartierung", tr = tr
  )
  output$download_type_map_svg <- create_leaflet_png_handler(
    reactive({ build_export_type_map() }), "SeriARC_Typenkartierung_SVG", tr = tr
  )
